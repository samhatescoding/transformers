...
\item
\iten \textbf{Benchmark Types}
\item Type A: Visual Questions and Answers
\item Tybe B: Bounding Box Detection...
\item \textbf{Results: GPT-4o vs Claude Sonnet 3.5}
\item
\item
\end{enumerate}
...
\includegraphics[width=\textwidth,height=0.62\textheight,keepaspectratio]
  {vlm_vs_llm_transformer_block.pdf}
\caption{A vision-language model extends the language-model pipeline by adding
a visual token stream. Image features and text tokens are fused in the
Transformer before the model generates a text output.}
\label{fig:vlm-vs-llm}
\end{figure}
...
\subsection*{R: Rating -- TAD66K}

Rating benchmarks ask the model to assign a numerical judgement to an image.
In TAD66K, the model rates the overall aesthetic quality of the image on a
1--10 scale. This is a Type R benchmark because the expected output is exactly
one integer, and the evaluator can measure either exact agreement or absolute
error from the reference rating.

\subsection*{P: Preference -- Pick-a-Pic}

Preference benchmarks ask the model to compare two images and choose the
preferred one. In Pick-a-Pic, the two images are shown as Image A and Image B,
and the model must decide which is more aesthetically pleasing overall. This is
a Type P benchmark because the expected output is a binary preference, returned
as either \code{A} or \code{B}.

\input{images_benchmark_types/five_examples_fragment.tex}

\begin{table}[H]
\centering
\small
\caption{The eight benchmark types covered in this note.}
...
R & Rating & TAD66K & One integer aesthetic score from 1 to 10. \\
P & Preference & Pick-a-Pic & One binary image preference, \code{A} or \code{B}. \\
\bottomrule
\end{tabularx}
\end{table}
