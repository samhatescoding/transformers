...
def main() -> None:
    ...
    parser.add_argument(
        "--train-manifest",
        type=Path,
        default=Path("fine_tuning/data/fashion_mnist/train_manifest.jsonl"),
    )
    parser.add_argument(
        "--validation-manifest",
        type=Path,
        default=Path("fine_tuning/data/fashion_mnist/validation_manifest.jsonl"),
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("fine_tuning/output/qwen2.5-vl-3b-fashion-mnist-lora"),
    )
    ...
    for manifest_path in (args.train_manifest, args.validation_manifest):
        if not manifest_path.is_file():
            raise SystemExit(
                f"Missing {manifest_path}. Run fine_tuning/prepare_fashion_mnist.py first."
            )
