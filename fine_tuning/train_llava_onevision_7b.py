"""Fine-tune LLaVA-OneVision Qwen2 7B with LoRA on image-text manifests."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        choices=("none", "4bit"),
        default="4bit",
        help="Use 4-bit QLoRA by default; select none for BF16 LoRA.",
    )
    parser.add_argument(
        "--train-vision-tower",
        action="store_true",
    ...
    for manifest_path in (args.train_manifest, args.validation_manifest):
        if not manifest_path.is_file():
            raise SystemExit(
                ...
                "Create it with fine_tuning/prepare_benchmark.py."
            )
    if args.epochs <= 0:
...
def freeze_vision_adapter_parameters(model: Any) -> int:
    ...
    for name, parameter in model.named_parameters():
        lowered = name.lower()
        if "vision_tower" in lowered or "vision_model" in lowered:
...
def main() -> None:
    ...
    if not torch.cuda.is_available():
        raise SystemExit("LLaVA-OneVision fine-tuning requires a CUDA GPU.")
    ...
    trainer = Trainer(
        ...
        data_collator=LlavaOnevisionCollator(processor),
        processing_class=processor,
    )
    trainer.train(resume_from_checkpoint=args.resume_from_checkpoint)
    trainer.save_model(str(args.output_dir))
    processor.save_pretrained(str(args.output_dir))
    print(f"Saved LLaVA-OneVision LoRA adapter and processor to {args.output_dir}")
