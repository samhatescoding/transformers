...
class Baichuan2(BaseModel):
    def __init__(
        self,
        model_id: str = "baichuan-inc/Baichuan2-7B-Chat",
        max_new_tokens: int = 100,
        temperature: float = 0.0,
    ):
        self.name = "baichuan2"
        self.model_id = "baichuan-inc/Baichuan2-7B-Chat" if model_id == "baichuan2" else model_id
        self.max_new_tokens = max_new_tokens
        self.temperature = temperature

        print("Loading Baichuan2 tokenizer...")
        self.tokenizer = AutoTokenizer.from_pretrained(self.model_id, use_fast=False)
        if self.tokenizer.pad_token_id is None:
            self.tokenizer.pad_token_id = self.tokenizer.eos_token_id

        print("Loading Baichuan2 model...")
    ...
    def _adapt_prompt(prompt: str) -> str:
        # Baichuan2 is text-only. Drop multimodal markers and keep user text.
        p = prompt.replace("<image>", "")
        p = p.replace("USER:", "").replace("ASSISTANT:", "").strip()
        return p or "Describe the input."

    def predict(self, image: Image.Image, prompt: str) -> str:
        del image  # Interface compatibility: Baichuan2 does not consume images.
